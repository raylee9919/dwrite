...
#define _UNICODE
#include <windows.h>
#include <dwrite.h> // @NOTE: "dwrite_2.h" minimum: Windows 8.1
#include <d2d1.h>
...
wWinMain(HINSTANCE hinst, HINSTANCE /*hprevinst*/, PWSTR /*pCmdLine*/, int /*nCmdShow*/)
{
    ...
    LPWSTR font_family_name = L"consolas";
    ...
    win32_assume_hr(font_collection->FindFamilyName(font_family_name, &family_index, &family_exists));
    assert(family_exists);

    win32_assume_hr(font_collection->GetFontFamily(family_index, &font_family));

    win32_assume_hr(font_family->GetFirstMatchingFont(DWRITE_FONT_WEIGHT_REGULAR, DWRITE_FONT_STRETCH_NORMAL, DWRITE_FONT_STYLE_NORMAL, &font));
    ...
    // @NOTE: LOCALE_NAME_MAX_LENGTH := 85, which includes a terminating null character.
    LPWSTR locale[LOCALE_NAME_MAX_LENGTH];
    int size = GetUserDefaultLocaleName(locale, LOCALE_NAME_MAX_LENGTH);
    if (! size) 
    { memcpy(locale, L"en-US", sizeof(L"en-US")); }
